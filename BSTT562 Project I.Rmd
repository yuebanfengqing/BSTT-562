...
```{r, echo = T}
## make a table
x <- matrix(round(c(x_hat, variance, cll, clu, scl_low, x_hat2, variance2, cll20, clu20, NA,
              x_hat3, variance3, cll100, clu100, NA), 4), 
            3, 5, byrow = T)
x <- as.matrix(cbind(c('0 ug/L', '20 ug/L', '100 ug/L'), x))
colnames(x) <- c('True Concentration','X_hat', 'Var(X)', 'Lower Bound', 'Upper Bound', 'SCL')
