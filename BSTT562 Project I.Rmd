...
(1) Let $z_{ijk} = \frac{(y_{ijk}-\alpha_i)}{x_j}$ and $u_{ijk}=\frac{e_{ijk}}{xj}$. Using the estimate $\hat{\alpha_i}$ of $\alpha_i$ we compute the $z_{ijk}$'s by

$$
\frac{z_{ijk}}{\beta_i} = e^{\eta_{ijk}} + \frac{u_{ijk}}{\beta_i}.
$$
